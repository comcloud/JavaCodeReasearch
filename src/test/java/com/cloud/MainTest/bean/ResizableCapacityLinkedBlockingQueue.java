...
public class ResizableCapacityLinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    public void put(E e) throws InterruptedException {
        ...
        final int c;
        final ResizableCapacityLinkedBlockingQueue.Node<E> node = new ResizableCapacityLinkedBlockingQueue.Node<E>(e);
        final ReentrantLock putLock = this.putLock;
     ...
     */
    void unlink(ResizableCapacityLinkedBlockingQueue.Node<E> p, ResizableCapacityLinkedBlockingQueue.Node<E> pred) {
        // assert putLock.isHeldByCurrentThread();
        // assert takeLock.isHeldByCurrentThread();
    ...
    public boolean remove(Object o) {
        ...
        fullyLock();
        try {
            for (ResizableCapacityLinkedBlockingQueue.Node<E> pred = head, p = pred.next;
                 p != null;
                 pred = p, p = p.next) {
                if (o.equals(p.item)) {
                    unlink(p, pred);
     ...
     */
    ResizableCapacityLinkedBlockingQueue.Node<E> succ(ResizableCapacityLinkedBlockingQueue.Node<E> p) {
        if (p == (p = p.next))
            p = head.next;
        return p;
    }
    ...
    private class Itr implements Iterator<E> {
        private ResizableCapacityLinkedBlockingQueue.Node<E> next;           // Node holding nextItem
        private E nextItem;             // next item to hand out
        private ResizableCapacityLinkedBlockingQueue.Node<E> lastRet;
        private ResizableCapacityLinkedBlockingQueue.Node<E> ancestor;       // Helps unlink lastRet on remove()
        ...
        public E next() {
            ResizableCapacityLinkedBlockingQueue.Node<E> p;
            if ((p = next) == null)
        ...
        public void forEachRemaining(Consumer<? super E> action) {
            ...
            do {
                ...
                try {
                    if (es == null) {
                        p = p.next;
                        for (ResizableCapacityLinkedBlockingQueue.Node<E> q = p; q != null; q = succ(q))
        ...
        public void remove() {
            ResizableCapacityLinkedBlockingQueue.Node<E> p = lastRet;
            if (p == null)
    ...
    public Spliterator<E> spliterator() {
        return new ResizableCapacityLinkedBlockingQueue.LBQSpliterator();
    }
     ...
     */
    void forEachFrom(Consumer<? super E> action, ResizableCapacityLinkedBlockingQueue.Node<E> p) {
     ...
     */
    ResizableCapacityLinkedBlockingQueue.Node<E> findPred(ResizableCapacityLinkedBlockingQueue.Node<E> p, ResizableCapacityLinkedBlockingQueue.Node<E> ancestor) {
        // assert p.item != null;
        if (ancestor.item == null)
            ancestor = head;
        // Fails with NPE if precondition not satisfied
        for (ResizableCapacityLinkedBlockingQueue.Node<E> q; (q = ancestor.next) != p; )
            ancestor = q;
        return ancestor;
    }
    ...
    private boolean bulkRemove(Predicate<? super E> filter) {
        ...
        ResizableCapacityLinkedBlockingQueue.Node<E> p = null, ancestor = head;
        ResizableCapacityLinkedBlockingQueue.Node<E>[] nodes = null;
        int n, len = 0;
        do {
                ...
                if (nodes == null) {  // first batch; initialize
                    ...
                    for (ResizableCapacityLinkedBlockingQueue.Node<E> q = p; q != null; q = succ(q))
                        if (q.item != null && ++len == 64)
                            break;
                    nodes = (ResizableCapacityLinkedBlockingQueue.Node<E>[]) new ResizableCapacityLinkedBlockingQueue.Node<?>[len];
                }
                for (n = 0; p != null && n < len; p = succ(p))
                    nodes[n++] = p;
            } finally {
            ...
            if (deathRow != 0) {
                ...
                try {
                    for (int i = 0; i < n; i++) {
                        final ResizableCapacityLinkedBlockingQueue.Node<E> q;
                        if ((deathRow & (1L << i)) != 0L
                                && (q = nodes[i]).item != null) {
